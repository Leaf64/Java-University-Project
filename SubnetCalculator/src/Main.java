import static printPackage.Printer.*;
...
public class Main {
    ...
    public static void main(String[] args) throws Exception {
        if (args.length == 1) //if user will give argument
        {
            ...
            try {
                Integer.parseInt(IP_and_MASK[1]);
            ...
            if (Integer.parseInt(IP_and_MASK[1]) <= 32) {  //checking the mask is in good format
                if (matcher.matches()) {    //checking IP match regex
                    ...
                    //if (IP_and_MASK[0].equals(inetAddress.getHostAddress())) {
    ...
    public static void calculate(String IP, String mask) throws IOException {
        ...
        if (isItHostAddress(octetsIntfirstHost, octetsIntLastHost, octetsInt)) {
            askUserForPing(IP, mask);
        }
    ...
    public static boolean isItHostAddress(int[] firstHost, int[] lastHost, int[] octetsIP) {

        String firstHostString = "";
        String lastHostString = "";
        String octetsIPString = "";

        for (int i = 0; i < 4; i++) {
            firstHostString = firstHostString + firstHost[i];
            lastHostString = lastHostString + lastHost[i];
            octetsIPString = octetsIPString + octetsIP[i];
        }
        if (Integer.parseInt(octetsIPString) >= Integer.parseInt(firstHostString) && Integer.parseInt(octetsIPString) <= Integer.parseInt(lastHostString)) {
            return true;
        }
        return false;
    }
    ...
    public static void askUserForPing(String IPaddress, String mask) throws IOException {
